...
public class ClientMatrix {
    ...
    private SincronizacaoSalasMatrix sincronizacaoMatrix;

    public ClientMatrix(SessaoMatrix sessao) {
        this.host = SBCore.getConfigModulo(FabConfigApiMatrixChat.class).getPropriedade(FabConfigApiMatrixChat.URL_MATRIX_SERVER);
        this.httpHelper = new HttpHelper();
        this.sincronizacaoMatrix = new SincronizacaoSalasMatrix(sessao, httpHelper);
        if (!host.endsWith("/")) {
            this.host += "/";
        }
    }
    ...
    public void login(String userToken, LoginCallback onResponse) throws IOException {
        ...
        httpHelper.sendRequestAsync(host, HttpHelper.URLs.whoami, null, "GET", data -> {
            ...
            if (object.has("user_id")) {
                ...
                sincronizacaoMatrix.startSyncee();
            } else {
    ...
    public void registerRoomEventListener(ItfListenerEventoMatrix event) {
        sincronizacaoMatrix.addRoomEventListener(event);
    }

    public void removeRoomEventListener(ItfListenerEventoMatrix event) {
        sincronizacaoMatrix.removeRoomEventListener(event);
    }
